...
LOAD DATA INFILE '/usr/local/mysql-files/gdp2020.csv'
INTO TABLE gdp_2020
...
LOAD DATA INFILE '/usr/local/mysql-files/covid19.csv'
INTO TABLE covid_19
FIELDS TERMINATED BY ',' 
ENCLOSED BY '"' 
LINES TERMINATED BY '\n'
...
ORDER BY 
    ...
    -- TASK 4
    -- DISPLAYING COUNTRY NAMES,GDP GROWTH PERCENTAGE AND COVID DEATH COUNTS FOR COUNTRIES THAT EXPERIENCED A GDP FALL SHOWING THE WORST AFFECTED FIRST.
    SELECT
		g.Country,
    ...
    ORDER BY 
        ...
        DESCRIBE covid_19;
        -- TASKK 5
        -- FINDING COUNTRIES THAT ARE EITHER IN 10% OF GDP PER CAPITA OR IN THE TOP 10% OF COVID DEATHS, BUT NOT BOTH.
        -- FIRST CALCULATE 90TH PERCENTILE VALUES
    SELECT
g.Country,
PPP_gdp_per_capita,
COVID_Deaths
FROM gdp_2020
INNER JOIN covid_19
ON gdp_2020.country = covid_19.country	
LIMIT 1000;	

-- GDP growth rate & Covid deaths  
USE global_analysis;
...
-- With the rate of deaths in the countries the GDP_PER_CAPITA of the  countries is seen to depreciate.
